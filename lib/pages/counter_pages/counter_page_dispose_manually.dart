...
final counterDisposeManuallyProvider = StateProvider((ref) => 0);


// * In order to get access to providers, we need to make our widgets into ConsumerWidget
class CounterPageManuallyDisposeState extends ConsumerWidget {
  const CounterPageManuallyDisposeState({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    // * We can use the ref, to obtain like watch/read/listen/invalidate/reset individual providers

    final int counter = ref.watch(counterDisposeManuallyProvider);

    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: const Text('COUNTER DISPOSE MANUALLY'),
        actions: [
          IconButton(
              onPressed: (){
                ref.refresh(counterDisposeManuallyProvider);
              },
        ...
        child: Row(
          ...
          children: [
            FloatingActionButton(
              ...
              onPressed: () => ref.read(counterDisposeManuallyProvider.notifier).state--,
